IF OBJECT_ID('PS_SD040_06') IS NOT NULL
BEGIN
	DROP PROC PS_SD040_06
END
GO
--EXEC PS_SD040_06 '20101116001'
CREATE PROC PS_SD040_06
(
	@PackNo NVARCHAR(100)
)
AS
BEGIN
	SELECT 
		...
		(SELECT WhsName FROM [OWHS] WHERE WhsCode = (SELECT WhsCode FROM OIBT WHERE ItemCode = PS_PP090L.U_ItemCode AND BatchNum = PS_PP090L.U_LotNo AND Quantity = PS_PP090L.U_Weight)) AS WhsName,
		'O' AS Status,
		'' AS LineId		
	FROM 
		[@PS_PP090H] PS_PP090H
