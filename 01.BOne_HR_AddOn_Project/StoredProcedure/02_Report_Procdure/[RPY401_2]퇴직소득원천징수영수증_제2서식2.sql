CREATE  PROC RPY401_2
 ...
 AS
    ...
    ===========================================================================================*/
	--DROP PROC RPY401_2
	--Exec RPY401_2 '2010-01-01', '2010-12-31', '%', '%', '%', '%'
...
-- <2.정산자료 조회 >ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ    
	INSERT	INTO [#RPY401]
	SELECT	DocNum	   	=	T0.Code,
			U_MSTCOD	=	T0.U_MSTCOD,
			U_MSTNAM	=	T0.U_MSTNAM,
			U_EmpID		=	T0.U_EmpID,
			U_INTGBN	=	T5.U_INTGBN,
			U_DWEGBN	=	T5.U_DWEGBN,
			U_FRGTAX	=	T5.U_FRGTAX,
			U_FRGCOD	=	T2.HomeCountr,
			U_BUBCHK	=	'',
			U_BUSNUM	=	ISNULL(T4.U_BusNum, ''),
			U_CLTNAM	=	ISNULL(T4.U_CLTName, ''),
			U_COMPRT	=	ISNULL(T4.U_ComPrt, ''),
			U_COMNBR	=	ISNULL(T4.U_PerNum, ''),
...
--			U_EN2RET	=	T0.U_EN2RET,
	...
	FROM	[@PH_PY115A] T0
			...
			INNER JOIN [@PH_PY001A] T5 ON T0.U_MSTCOD = T5.Code --U_MSTCOD
			LEFT  JOIN [@PH_PY005A] T4 ON T4.CODE = T2.U_CLTCOD
