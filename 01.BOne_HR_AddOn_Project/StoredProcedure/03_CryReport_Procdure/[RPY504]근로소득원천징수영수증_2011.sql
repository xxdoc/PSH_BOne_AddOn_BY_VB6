IF(EXISTS(SELECT NAME FROM sysobjects WHERE NAME = 'RPY504_11' AND xtype = 'P'))
	DROP PROCEDURE RPY504_11
GO

CREATE PROC RPY504_11
    (
 ...
 AS
    /*==========================================================================================
        ...
        작업목적        : 
        작업내용        : 
        최종수정일			: 2011.11.21 2010년이후 필드추가분있음 이전분이랑 리포트차이 프로시저분리
    ===========================================================================================*/
    --DROP PROC RPY504_11
    --Exec RPY504_11  '2011', '01', '12', '3', N'%', N'%',  N'%'
    SET NOCOUNT ON
...
---------------------------------------------------------------------------------------------------
    ...
    CREATE TABLE #RPY504 (
		...
		U_PILHU2    Numeric(19,6),
        U_PILJHE    Numeric(19,6),
		U_PILJH2    Numeric(19,6),
		U_PILJH3    Numeric(19,6),
...
---------------------------------------------------------------------------------------------------
	...
	ELSE IF @JSNYER >= '2009'
		BEGIN
			...
			SELECT  @BTXNAM_G01 =  MAX(CASE WHEN T0.U_BTXCOD = 'G01' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_H01 =  MAX(CASE WHEN T0.U_BTXCOD = 'H01' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_H05 =  MAX(CASE WHEN T0.U_BTXCOD = 'H05' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_H06 =  MAX(CASE WHEN T0.U_BTXCOD = 'H06' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_H07 =  MAX(CASE WHEN T0.U_BTXCOD = 'H07' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_H08 =  MAX(CASE WHEN T0.U_BTXCOD = 'H08' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_H09 =  MAX(CASE WHEN T0.U_BTXCOD = 'H09' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_H10 =  MAX(CASE WHEN T0.U_BTXCOD = 'H10' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_H11 =  MAX(CASE WHEN T0.U_BTXCOD = 'H11' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_H12 =  MAX(CASE WHEN T0.U_BTXCOD = 'H12' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_H13 =  MAX(CASE WHEN T0.U_BTXCOD = 'H13' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_I01 =  MAX(CASE WHEN T0.U_BTXCOD = 'I01' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_K01 =  MAX(CASE WHEN T0.U_BTXCOD = 'K01' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_M01 =  MAX(CASE WHEN T0.U_BTXCOD = 'M01' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_M02 =  MAX(CASE WHEN T0.U_BTXCOD = 'M02' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_M03 =  MAX(CASE WHEN T0.U_BTXCOD = 'M03' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_O01 =  MAX(CASE WHEN T0.U_BTXCOD = 'O01' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_Q01 =  MAX(CASE WHEN T0.U_BTXCOD = 'Q01' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_S01 =  MAX(CASE WHEN T0.U_BTXCOD = 'S01' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_T01 =  MAX(CASE WHEN T0.U_BTXCOD = 'T01' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_X01 =  MAX(CASE WHEN T0.U_BTXCOD = 'X01' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_Y01 =  MAX(CASE WHEN T0.U_BTXCOD IN ('Y01','R10') THEN U_CSUNAM ELSE '' END),
					@BTXNAM_Y02 =  MAX(CASE WHEN T0.U_BTXCOD = 'Y02' THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_Y03 =  MAX(CASE WHEN T0.U_BTXCOD = 'Y03' THEN U_CSUNAM ELSE '' END),
					@BTXNAM_Y20 =  MAX(CASE WHEN T0.U_BTXCOD IN ('Y20','Y22') THEN U_CSUNAM ELSE '' END), 
					@BTXNAM_Z01 =  MAX(CASE WHEN T0.U_BTXCOD = 'Z01' THEN U_CSUNAM ELSE '' END)
			FROM    [@PH_PY102B] T0 
					INNER JOIN [@PH_PY102A] T1 ON T0.Code = T1.Code
			WHERE   LEFT(T1.U_YM,4) = (SELECT LEFT(MAX(U_YM),4) FROM [@PH_PY102A] WHERE LEFT(U_YM,4) <= @JSNYER) 
			AND     LEFT(T1.U_YM,4) >= '2009' 
			...
			SET @BTXNAM_Y01 =  ISNULL(@BTXNAM_Y01,CASE WHEN @JSNYER = '2009' THEN N'우리사주 배정' ELSE N'근로장학금' END)
			SET @BTXNAM_Y02 =  ISNULL(@BTXNAM_Y02,N'우리사주 인출')
			SET @BTXNAM_Y03 =  ISNULL(@BTXNAM_Y03,N'우리사주 인출')
			SET @BTXNAM_Y20 =  ISNULL(@BTXNAM_Y20,CASE WHEN @JSNYER = '2009' THEN N'주택자금보조금' ELSE N'지정비과세' END)
...
---------------------------------------------------------------------------------------------------
    ...
    SELECT  U_MSTCOD    =   T0.U_MSTCOD,
            ...
            U_STRINT    =   CASE WHEN T0.U_CLTCOD <> T2.U_CLTCOD THEN CONVERT(CHAR(10),T0.U_STRINT,120)
								 WHEN T2.U_StartDat > T0.U_STRINT THEN CONVERT(CHAR(10),T2.U_StartDat,120) 
            ...
            U_BIGWA2    =   T0.U_BIGWA2+ISNULL(T0.U_BIGWA7,0),
			...
			U_PILHU2    =   ISNULL(T0.U_PILHU2,0),
            U_PILJHE    =   T0.U_PILJHE,
			U_PILJH2    =   ISNULL(T0.U_PILJH2, 0),
			U_PILJH3    =   ISNULL(T0.U_PILJH3, 0),
            ...
            U_DAGYSU    =   ISNULL(T1.U_DAGYSU,0),
            U_CHLSAN    =   ISNULL(T1.U_CHLSAN,0),
            U_PERNBR    =   ISNULL(T2.U_GovID, ''),
            U_ADDRES    =   ISNULL(T2.U_Address, ''),
			...
			U_GUKNAM	=	CASE WHEN T1.U_INTGBN = '1' THEN '' ELSE (SELECT A0.Name FROM OCRY A0 WHERE A0.Code = ISNULL(T2.U_brthCntr, '')) END,
			U_GUKCOD	=	CASE WHEN T1.U_INTGBN = '1' THEN '' ELSE ISNULL(T2.U_brthCntr, '') END,
    ...
    FROM    [@ZPY504H] T0   
            ...
            --LEFT JOIN [MDC_PAYPIC] T9 ON T9.EmpID = T2.U_CLTCOD * -1
    WHERE   T0.U_JSNYER =    @JSNYER
    AND     T0.U_CLTCOD LIKE @CLTCOD
