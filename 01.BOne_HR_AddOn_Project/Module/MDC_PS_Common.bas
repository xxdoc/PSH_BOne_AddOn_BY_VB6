...
On Error GoTo User_BPLId_Error
    ...
    sQry = "Select Branch From [OUSR] Where USER_CODE = '" & Trim(oCompany.UserName) & "'"
...
'    sQry = "Select U_MSTCOD From [OHEM] a Inner Join [OUSR] b On a.userId = b.USERID Where b.USER_CODE = '" & Trim(Sbo_Company.UserName) & "'"
    sQry = "Select U_MSTCOD From [OHEM] a Inner Join [OUSR] b On a.userId = b.USERID Where b.USER_CODE = '" & Trim(oCompany.UserName) & "'"
    oRecordSet01.DoQuery sQry
...
'해당모듈 : MDC_PS_Common
'기능 : KPI 평가등급 계산
'인수 : prmBaseEntry(KPI목표문서번호), prmBaseLine(KPI목표문서행번호), prmTableName(KPI목표 테이블 명), prmResult(실적), prmMonth(실적등록월)
'반환값 : KPI평가등급
'특이사항 : 없음
'******************************************************************************
...
'해당모듈 : MDC_PS_Common
'기능 : KPI 진척율(달성율)
'인수 : prmBasEntry(목표문서번호), prmBasLine(목표행번호), prmDocType(문서타입), prmRslt(실적)
'반환값 : KPI평가점수
'특이사항 : 없음
'******************************************************************************
